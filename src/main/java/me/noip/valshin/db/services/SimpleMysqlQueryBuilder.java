...
public class SimpleMysqlQueryBuilder implements MysqlQueryBuilder {
	...
	private String noteUpdateQuery = "UPDATE `notes` WHERE `id` = '%d' SET `name` = '%s', `second_name` = '%s', `last_name` = '%s', `phone` = '%s'";
	...
	public String noteWrite(long owner, Note note) {
		return getMainNoteQuery(noteWriteQuery, owner, note);
	}
	...
	public String noteUpdate(long id, Note note) {
		return getMainNoteQuery(noteUpdateQuery, id, note);
	}
	...
	private String getMainNoteQuery(String query, long id, Note note){
